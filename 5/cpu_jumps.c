...
static int jump_vec[4096] = {0};

static size_t do_jumps(size_t len, int* vec) {
	int* cur = vec;
	size_t result = 0;
	while(cur >= vec && cur - vec < len) {
		result++;
		int* toPrint = vec;

		/*
		printf("%zu: ", result);
		while(toPrint < vec + len) {
			if (toPrint == cur) {
				printf("(%d) ", *toPrint);
			}
			else {
...
int main(int argc, const char** argv) {
	...
	size_t len = 0;
	while(1 == fscanf(in, "%d", cur++));
	cur--;
	len = cur - jump_vec;
	printf("Read %zu numbers\n", len);

	size_t result = do_jumps(len, jump_vec);
	printf("%zu\n", result);
	return 0;
}
